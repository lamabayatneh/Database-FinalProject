...
public class PurchaseDAO {
	...
	public static int insertPurchaseReturnId(Connection con, int supplierId, int staffId, LocalDate date,
	...
	public static List<Purchase> getAllPurchases() {
		...
		try (Connection con = DBConnection.getConnection();
			...
			while (rs.next()) {
				Supplier sup = new Supplier(rs.getInt("SupplierID"), rs.getString("SupplierName"), rs.getString("City"),
						rs.getString("Email"), rs.getString("Phone"), rs.getString("ContactPerson"));

				list.add(new Purchase(rs.getInt("PurchaseID"), sup, rs.getInt("StaffID"),
						rs.getDate("PurchaseDate").toLocalDate(), rs.getDouble("TotalCost")));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
