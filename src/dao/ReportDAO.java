...
public class ReportDAO {
	...
	public static List<MonthlyFinanceRow> getMonthlyFinance() {
		...
		try (Connection con = DBConnection.getConnection();
			...
			while (rs.next()) {
				list.add(new MonthlyFinanceRow(rs.getString("Month"), rs.getDouble("SalesRevenue"),
						rs.getDouble("PurchaseExpenses"), rs.getDouble("Profit"), rs.getDouble("ProfitMargin")));
			}
	...
	public static CategoryRevenueRow getTopCategoryBySalesRevenue() {
		...
		try (Connection con = db.DBConnection.getConnection();
			...
			if (rs.next()) {
				return new CategoryRevenueRow(rs.getInt("CategoryID"), rs.getString("CategoryName"),
	...
	public static SupplierCostRow getTopSupplierByPurchaseCost() {
		...
		try (Connection con = db.DBConnection.getConnection();
			...
			if (rs.next()) {
				return new SupplierCostRow(rs.getInt("SupplierID"), rs.getString("SupplierName"), rs.getDouble("cost"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
